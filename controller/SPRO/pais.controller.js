...
sap.ui.define([
 ...
 "sap/m/MessageToast"
   ...
   "use strict";
   return BaseController.extend("sap.ui.su01.controller.SPRO.pais", {

    onInit: function(oEvent){

      var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
      oRouter.getRoute("pais").attachPatternMatched(this._onObjectMatched, this);
    ...
    _onObjectMatched: function(oEvent) {
      ...
      $.ajax({
            ...
            success:  function (response) {    
                thes.propiedadesInput(thes,response,"pais","tpais");
            },
    ...
    onSave: function(oEvent){
      ...
      var datosTabla =  this.byId("tbPaises").getModel().getData();
      ...
      for (var i = 0 ; i <= ultimo; i++) {
        ...
        if (datos['data'][0][i]['codPais'] == ""){
          ...
          result[i] = 0;  
          MessageToast.show("Rellene el campo código");
      ...
      if (flag == "X") { 
          ...
          var newTable = this.byId("tbPaises").getModel().getData();

          for (var i = 0; i <= newTable.length -1; i++) {
            thes.byId("tbPaises").mAggregations.rows[i].mAggregations.cells[0].setEditable(false);
          }
    ...
    cargarPais:function(thes){   
      ...
      $.ajax({
        ...
        success:  function (response) {    
          ...
          thes.byId("tbPaises").setModel(oModel);
    ...
    $.ajax({
            url:   '/erp/model/entidades/pais.json', 
            type:  'GET',
            async: false,
            beforeSend: function () {
            },
            success:  function (response) {    
              var data = thes.byId("tbPaises").getModel().getData();
              data.push(response);
              thes.byId("tbPaises").getModel().refresh();
            },
            error: function (xhr, ajaxOptions, thrownError) {
              alert(xhr.status);
              alert(thrownError);
    ...
    var cant = this.byId("tbPaises").getModel().getData().length - 1;

    thes.byId("tbPaises").mAggregations.rows[cant].mAggregations.cells[0].setEditable(true);
    ...
    onDelete:function(oEvent){
     ...
     var thes = this;           
     var datosTabla = this.byId("tbPaises").getModel().getData();
     var selectData = this.byId("tbPaises").getSelectedIndices();
     var index;
     ...
     this.byId("tbPaises").getModel().refresh();
     this.byId("tbPaises").removeRow();
